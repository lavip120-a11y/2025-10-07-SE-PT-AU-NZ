...
describe("Calculator Routes", () => { //describes all routes related to calculator so only need it once
  //describe the test cases for calculator routes
  ...
  test("GET /calculate/add => sum of numbers", () => {
    return request(app) //returns the supertest request object and passing the entire operation to it and returning it
      ...
      .then((response) => {
        //handling the response using then method
        expect(response.body).toEqual({
          //expecting the body of the response to be equal to
          result: number1 + number2, //sum of the two numbers
        });
      });
  });

  test("Get /calculate/subtract => subtract of numbers", () => { //adding a test case for subtract route
    return request(app) //chaining methods
